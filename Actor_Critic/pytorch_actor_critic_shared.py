...
class actor_critic(nn.Module):
    ...
    def __init__(self, state_space, action_space, seed, first_layer=16, second_layer=16):
        ...
        '''shared input layers'''
        self.input_layer = nn.Linear(state_space, first_layer).to(device)
        self.middle_nn2 = nn.Linear(first_layer, second_layer).to(device)

        self.pol_nn3 = nn.Linear(second_layer, action_space).to(device)
    ...
    def forward(self, state, output_choice):
        ...
        if (output_choice == "policy"):
            ...
            output = F.relu(self.middle_nn2(output))
            output = F.softmax(self.pol_nn3(output), dim=1)
        else:
            '''value'''
