...
class Agent():
    ...
    def learn(self, experience, gamma):
        ...
        sampled_state, sampled_action, sampled_reward, sampled_next_state, sampled_done, index = experience
        ...
        #TODO 1. Need to see if this is casuing slowdown. The code is very slow compared to regular sampling.

        delta = np.float32(np.abs(DQN_estimation.detach().cpu().numpy() - DQN_target.detach().cpu().numpy()))

        '''update priority weight with delta'''

        '''call function use delta, and index as inputs '''
        self.memory.add_priority(delta, index)
