...
class QDQN(nn.Module):
    ...
    def __init__(self, state_space, action_space, hidden_layer_1=128, hidden_layer_2=128, n_quantiles=51):
        ...
        self.noisy_layer = NoisyLinear(hidden_layer_1, hidden_layer_2)
        self.noisy_layer1 = NoisyLinear(hidden_layer_2, action_space * n_quantiles)

    def forward(self, state):

        output = F.relu(self.linear(state))
        output = F.relu(self.noisy_layer(output))
        output = self.noisy_layer1(output)
